...
module.exports = class ViewportController {
  ...
  point(hand, frame) {
    ...
    vel[1] = -vel[1];
    if (this.viewPorts[this.currentViewport] instanceof PerspViewport) {
      this.viewPorts[this.currentViewport].pan(vel, 2);
      return;
    }
    this.viewPorts[this.currentViewport].moveCursor(vel, 1);
  }
  oneHandPinch(hand, frame) {
    const vel = hand.translation(frame);
    vel[1] = -vel[1];
    if (this.viewPorts[this.currentViewport] instanceof PerspViewport) {
      this.viewPorts[this.currentViewport].rotate(vel, 1);
      return;
    }
    this.viewPorts[this.currentViewport].pan(vel, 2);
  }
  openPalm(hand, frame) {
    const vel = hand.translation(frame);
    this.viewPorts[this.currentViewport].zoom(vel[2], 3);
  }
